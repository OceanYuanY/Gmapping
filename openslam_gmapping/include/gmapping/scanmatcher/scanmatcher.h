...
inline double ScanMatcher::score(const ScanMatcherMap& map, const OrientedPoint& p, const double* readings) const{ 
	...
	lp.x+=cos(p.theta)*m_laserPose.x-sin(p.theta)*m_laserPose.y;
	lp.y+=sin(p.theta)*m_laserPose.x+cos(p.theta)*m_laserPose.y;
	lp.theta+=m_laserPose.theta;//lp为全局参考系 激光雷达的位置（x,y,theta）
	unsigned int skip=0;
	double freeDelta=map.getDelta()*m_freeCellRatio;
	//r 距离
	for (const double* r=readings+m_initialBeamsSkip; r<readings+m_laserBeams; r++, angle++){
		skip++;
		skip=skip>m_likelihoodSkip?0:skip;
		if (skip||*r>m_usableRange||*r==0.0) continue;//去除距离为最大距离的
		Point phit=lp;
		phit.x+=*r*cos(lp.theta+*angle);
		phit.y+=*r*sin(lp.theta+*angle);//phit为测量终点在全局参考系的位置（x,y,theta）
		IntPoint iphit=map.world2map(phit);//格数坐标
		Point pfree=lp;
		pfree.x+=(*r-map.getDelta()*freeDelta)*cos(lp.theta+*angle);
		pfree.y+=(*r-map.getDelta()*freeDelta)*sin(lp.theta+*angle);
 		pfree=pfree-phit;//空闲网格与障碍网格的距离（欧式距离）
		IntPoint ipfree=map.world2map(pfree);//相差网格格数
		bool found=false;
		Point bestMu(0.,0.);
		//pr为障碍网格的八个周围网格 pf为空闲网格
		for (int xx=-m_kernelSize; xx<=m_kernelSize; xx++)
		for (int yy=-m_kernelSize; yy<=m_kernelSize; yy++){
			IntPoint pr=iphit+IntPoint(xx,yy);
			IntPoint pf=pr+ipfree;
