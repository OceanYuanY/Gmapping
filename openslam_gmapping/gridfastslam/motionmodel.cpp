...
MotionModel::drawFromMotion(const OrientedPoint& p, const OrientedPoint& pnew, const OrientedPoint& pold) const{
	double sxy=0.3*srr;
	OrientedPoint delta=absoluteDifference(pnew, pold);//对于old的绝对坐标（即old点的局部参考系的移动，三维，平移旋转（x,y,theta））
	OrientedPoint noisypoint(delta);//(x,y,theta) = (delta.x, delata.y, 0.0)
