...
void SlamGMapping::startLiveSlam()
{
  ...
  scan_filter_sub_ = new message_filters::Subscriber<laserscan_pro::LaserScanPro>(node_, "scanpro", 5);
  scan_filter_ = new tf::MessageFilter<laserscan_pro::LaserScanPro>(*scan_filter_sub_, tf_, odom_frame_, 5);
...
void SlamGMapping::startReplay(const std::string & bag_fname, std::string scan_topic)
{
  ...
  std::queue<std::pair<laserscan_pro::LaserScanPro::ConstPtr, std::string> > s_queue;
  foreach(rosbag::MessageInstance const m, viewall)
  {
    ...
    laserscan_pro::LaserScanPro::ConstPtr s = m.instantiate<laserscan_pro::LaserScanPro>();
    if (s != NULL) {
...
SlamGMapping::initMapper(const sensor_msgs::LaserScan& scan)
{
  ...
  gsp_->setUpdateDistances(linearUpdate_, angularUpdate_, resampleThreshold_);//最大地图更新数据
...
SlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose)
{
  if(!getOdomPose(gmap_pose, scan.header.stamp)){
    ROS_WARN("GMAPPING: CANNOT GET ODOM DATA");
    return false;
  }
  ...
  //如果角度增量是负数，即angle_min>angle_max时，更改数据顺序(no need)
...
void
SlamGMapping::laserCallback(const laserscan_pro::LaserScanPro::ConstPtr& scanpro)
{
  laser_count_++;

  sensor_msgs::LaserScan scan_;
  scan_.angle_increment = (*scanpro).angle_increment;
  scan_.angle_max = (*scanpro).angle_max;
  scan_.angle_min = (*scanpro).angle_min;
  scan_.header = (*scanpro).header;
  scan_.intensities = (*scanpro).intensities;
  scan_.range_max = (*scanpro).range_max;
  scan_.range_min = (*scanpro).range_min;
  scan_.ranges = (*scanpro).ranges;
  scan_.scan_time = (*scanpro).scan_time;
  scan_.time_increment = (*scanpro).time_increment;
  sensor_msgs::LaserScan* scan = &scan_;
  ...
  //角度计算转移至此
  // double theta = - std::fabs(scan_.angle_min - scan_.angle_max)/2;
  laser_angles_.clear();
  laser_angles_.resize(scan_.ranges.size());
  for(unsigned int i=0; i<scan_.ranges.size(); ++i)
  {
    laser_angles_[i]=(*scanpro).theta[i];
  }
